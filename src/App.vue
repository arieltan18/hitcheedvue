<template>
  <div id="app">
    ...
    <BrowseCategories :route-key="$route.params.category" v-if="(this.$route.name=='projectsByCategory') || (this.$route.name=='projectsByCategoryAll') || (this.$route.name=='projectsByAllCategories') || (this.$route.name=='projectsByTag')" style="padding-top:200px;"></BrowseCategories>
    <router-view :key="$route.fullPath" v-if="this.$route.path!='/'" :style="(this.$route.name == 'projectsByCategory') || (this.$route.name == 'projectsByCategoryAll') || (this.$route.name=='projectsByAllCategories') || (this.$route.name=='projectsByTag') ? 'padding-top:50px;' : 'padding-top:162px;'"/>
    <router-view v-else />
    <Footer />
  </div>
</template>
